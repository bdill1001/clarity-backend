...
async function updateRegistryFromAnalysis(artistId, artistName, aiLikelihood, artistImage = null) {
  // HARDENING GUARD: Never save an artist with a placeholder/loading name
  if (!artistName || artistName.includes('Analyzing') || artistName === '...') {
    console.log(`[Registry] Skipping ingestion for placeholder name: "${artistName}" (ID: ${artistId})`);
    return;
  }
  
  let points = 0;
  if (aiLikelihood >= 65) points = 10; // Instantly add to registry if identified as Likely AI
...
app.post('/api/analyze', async (req, res) => {
  ...
  try {
    ...
    let safeReasons = analysis.reasons;
    if (!Array.isArray(safeReasons)) safeReasons = ["Assessment complete."];
       ...
       if (!topRes.ok) {
           if (topRes.status === 403 || topRes.status === 404 || topRes.status === 429) {
               ...
               if (artistRes.ok) {
                   ...
                   console.warn(`[Registry Submit] Could not fetch artist name via API. Status: ${artistRes.status}. Attempting oEmbed bypass...`);
                   try {
                       const webpageRes = await fetch(`https://open.spotify.com/oembed?url=spotify:artist:${id}`);
                       if (webpageRes.ok) {
                           const oEmbedData = await webpageRes.json();
                           if (oEmbedData.title) artistName = oEmbedData.title;
                       }
                   } catch (scrapeErr) {
                       console.warn('[Registry Submit] oEmbed scrape failed:', scrapeErr.message);
                   }
               }
...
5. THE NUCLEAR INNOCENCE RULE: If you do not explicitly recognize this artist from your training data, YOU MUST DEFAULT to 'Unsure' or 'Likely Human' to protect obscure human artists. The ONLY exceptions allowing a 'Likely AI' classification are if the "Forensic Corroboration Gate" is met (At least TWO of the following signals must be positive):
   ...
   E) Shadowban Context: If the track name is "[Tracks Unavailable - Region Locked or Removed]", it means the artist was likely purged for AI. This is a very strong AI indicator.
...
Return ONLY a strict JSON object:
{
  ...
  "reasons": ["1-2 sentences explaining reasoning"]
    ...
    // Attempt standard parse, fallback to regex extraction if Gemini hallucinated prepended text
    let analysis;
    try {
      analysis = JSON.parse(textResp);
    } catch (parseError) {
      console.warn('[Gemini] JSON Parse failed, attempting regex extraction. Original text:', textResp);
    ...
    // Safety boundaries
    if (typeof analysis.aiLikelihood !== 'number') analysis.aiLikelihood = 50;
    if (!analysis.label) analysis.label = "Unsure";
    if (!Array.isArray(analysis.reasons)) analysis.reasons = ["Automated fallback assessment."];
        ...
        if (!notifiedTracksCache.has(cacheKey)) {
          ...
          const pushRes = await fetch('https://exp.host/--/api/v2/push/send', {
            ...
            body: JSON.stringify({
              ...
              title: '🚨 AI Song Detected!',
              body: `"${track.name}" by ${track.artists[0].name} sounds artificially generated. Check the analysis.`,
              data: { track: track, analysis: analysis },
              sound: 'default',
              priority: 'high',
            }),
          });
          ...
          if (pushRes.ok) {
            console.log(`[Worker] Push Notification sent successfully to User ${user.id}.`);
          } else {
            const pushErr = await pushRes.text();
            console.error(`[Worker] Push Notification FAILED for User ${user.id}:`, pushErr);
          }
        } else {
           console.log(`[Worker] Suppression HIT for User ${user.id} and Track ${trackId}. Notification skipped.`);
        }
      }
